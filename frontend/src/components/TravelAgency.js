...
function TravelAgency() {
  ...
  const loadTransactions = async () => {
    try {
      const response = await api.get(`/transactions/module/travel`, {
  ...
  const loadCustomers = async () => {
    try {
      ...
      const response = await api.get('/customers', {
        params: { limit: 1000, module: 'travel' }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post('/transactions', {
        ...
        travel_airline: formData.travel_airline,
        travel_passenger_name: formData.travel_passenger_name,
        travel_passport: formData.travel_passport,
        travel_pnr: formData.travel_pnr,
        travel_date: formData.travel_date,
        travel_ticket_issue_date: formData.travel_ticket_issue_date
      });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await api.put(`/transactions/${editTransaction.id}`, {
        ...
        travel_airline: editTransaction.travel_airline,
        travel_passenger_name: editTransaction.travel_passenger_name,
        travel_passport: editTransaction.travel_passport,
        travel_pnr: editTransaction.travel_pnr,
        travel_date: editTransaction.travel_date,
        travel_ticket_issue_date: editTransaction.travel_ticket_issue_date
      });
  ...
  return (
    <div className="module-container">
        ...
        <div className="form-card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-section">
              ...
              >
                <option value="debit">New Ticket Purchase</option>
              ...
              {formData.transaction_type === 'credit' && selectedCustomerBalance && (
                <div style={{
                  ...
                  <p style={{ margin: '0 0 8px 0', fontWeight: 'bold' }}>Customer Balance:</p>
                  <p style={{
                    margin: '0 0 8px 0',
                    fontSize: '18px',
                    color: selectedCustomerBalance.balance > 0 ? '#e53e3e' : '#38a169'
                  }}>
                    {selectedCustomerBalance.balance > 0 ? 'Outstanding: ' : 'Paid: '}
                    {Math.abs(selectedCustomerBalance.balance).toFixed(2)} PKR
                  </p>
                  {selectedCustomerBalance.balance > 0 && (
                    <button
                      type="button"
                      onClick={() => setFormData({...formData, amount: selectedCustomerBalance.balance.toString()})}
                      style={{
                    ...
                    >
                      Pay Full Amount ({selectedCustomerBalance.balance.toFixed(2)} PKR)
                    </button>
                  )}
                </div>
              )}
              ...
              {formData.transaction_type === 'debit' && (
                <>
                  ...
                  <input
                    type="text"
                    placeholder="Airline (e.g., PIA, Emirates, Qatar Airways)"
                    value={formData.travel_airline}
                    onChange={(e) => setFormData({...formData, travel_airline: e.target.value})}
                  />

                  <input
                    type="text"
                    placeholder="Passenger Name"
                    value={formData.travel_passenger_name}
                    onChange={(e) => setFormData({...formData, travel_passenger_name: e.target.value})}
                  />

                  <input
                    type="text"
                    placeholder="Passport Number"
                    value={formData.travel_passport}
                    onChange={(e) => setFormData({...formData, travel_passport: e.target.value})}
                  />

                  <input
                    type="text"
                    placeholder="PNR Number"
                    value={formData.travel_pnr}
                    onChange={(e) => setFormData({...formData, travel_pnr: e.target.value})}
                  />
                  ...
                  <input
                    type="date"
                    value={formData.travel_ticket_issue_date}
                    onChange={(e) => setFormData({...formData, travel_ticket_issue_date: e.target.value})}
                  />
                </>
              )}
      ...
      <div className="search-bar">
        <input
          type="text"
          placeholder="Search by customer name, phone, airline, PNR, or passenger..."
          value={searchTerm}
          ...
          <tbody>
            {transactions.map(t => (
              <tr key={t.id}>
                ...
                <td>{t.travel_airline || '-'}</td>
                <td>{t.travel_passenger_name || '-'}</td>
                <td>{t.travel_pnr || '-'}</td>
                <td>{t.travel_date ? new Date(t.travel_date).toLocaleDateString() : '-'}</td>
                <td>
                  <span style={{
                    padding: '4px 8px',
                    borderRadius: '4px',
          ...
          <div className="dialog edit-dialog">
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              <label>
                ...
                <input
                  type="text"
                  value={editTransaction.travel_airline || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, travel_airline: e.target.value})}
                />
              </label>
              ...
              <label>
                ...
                <input
                  type="text"
                  value={editTransaction.travel_passenger_name || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, travel_passenger_name: e.target.value})}
                />
              </label>
              ...
              <label>
                ...
                <input
                  type="text"
                  value={editTransaction.travel_passport || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, travel_passport: e.target.value})}
                />
              </label>
              ...
              <label>
                ...
                <input
                  type="text"
                  value={editTransaction.travel_pnr || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, travel_pnr: e.target.value})}
                />
              </label>
              ...
              <label>
                ...
                <input
                  type="date"
                  value={editTransaction.travel_date || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, travel_date: e.target.value})}
                />
              </label>
              ...
              <label>
                ...
                <input
                  type="date"
                  value={editTransaction.travel_ticket_issue_date || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, travel_ticket_issue_date: e.target.value})}
                />
              </label>
