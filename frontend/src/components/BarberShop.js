...
function BarberShop() {
  ...
  const loadTransactions = async () => {
    try {
      const response = await api.get(`/transactions/module/barber`, {
  ...
  const loadCustomers = async () => {
    try {
      ...
      const response = await api.get('/customers', {
        params: { limit: 1000, module: 'barber' }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post('/transactions', {
        ...
        barber_service: formData.barber_service,
        barber_staff: formData.barber_staff
      });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await api.put(`/transactions/${editTransaction.id}`, {
        ...
        barber_service: editTransaction.barber_service,
        barber_staff: editTransaction.barber_staff
      });
  ...
  return (
    <div className="module-container">
        ...
        <div className="form-card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-section">
              ...
              >
                <option value="debit">New Service</option>
              ...
              {formData.transaction_type === 'debit' && (
                <>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '10px' }}>
                    {['Haircut', 'Shave', 'Beard Trim', 'Hair Color', 'Facial', 'Hair Treatment', 'Head Massage'].map(service => (
                      <button
                        key={service}
                        type="button"
                        onClick={() => {
                          const current = formData.barber_service;
                          const newService = current ? `${current} + ${service}` : service;
                          setFormData({...formData, barber_service: newService});
                        }}
                        style={{
                          padding: '6px 12px',
                          fontSize: '13px',
                  ...
                  <textarea
                    placeholder="Services (e.g., Haircut + Shave + Facial)"
                    value={formData.barber_service}
                    onChange={(e) => setFormData({...formData, barber_service: e.target.value})}
                    rows="2"
                  />
                  ...
                  <input
                    ...
                    value={formData.barber_staff}
                    onChange={(e) => setFormData({...formData, barber_staff: e.target.value})}
                  />
                </>
              )}
      ...
      <div className="search-bar">
        <input
          type="text"
          placeholder="Search by customer name, phone, service, or staff..."
          value={searchTerm}
          ...
          <tbody>
            {transactions.map(t => (
              <tr key={t.id}>
                ...
                <td>{t.barber_service || '-'}</td>
                <td>{t.barber_staff || '-'}</td>
                <td>
                  <span style={{
                    padding: '4px 8px',
                    borderRadius: '4px',
          ...
          <div className="dialog edit-dialog">
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              <label>
                Service:
                <select
                  value={editTransaction.barber_service || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, barber_service: e.target.value})}
                >
              ...
              <label>
                ...
                <input
                  type="text"
                  value={editTransaction.barber_staff || ''}
                  onChange={(e) => setEditTransaction({...editTransaction, barber_staff: e.target.value})}
                />
              </label>
